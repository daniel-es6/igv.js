...
class AlignmentTrack extends TrackBase {
    ...
    computePixelHeight(alignmentContainer) {
        if (alignmentContainer.packedGroups) {
            ...
            const alignmentRowHeight =
                this.displayMode === "SQUISHED" ? this.squishedRowHeight : this.alignmentRowHeight
        ...
        function drawSingleAlignment(alignment, y, alignmentHeight) {
            ...
            if (alignment.insertions && this.showInsertions) {
                ...
                for (let insertionBlock of alignment.insertions) {
                    ...
                    const basePixelWidth =
                        !this.showInsertionText || insertionBlock.len === 1
                            ? 2
            ...
            for (let { bbox, baseColor, readChar } of basesToDraw) {
                ...
                if (bpPerPixel <= threshold && bbox.height >= 8) {
                    ...
                    IGVGraphics.strokeText(
                        ctx,
                        readChar,
                        center - ctx.measureText(readChar).width / 2,
                        fontHeight - 1 + bbox.y,
            ...
            if (this.colorBy && this.colorBy.startsWith("basemod")) {
                ...
                this.baseModRenderer.drawModifications(
                    alignment,
                    y,
                    alignmentHeight,
                    context,
                    this.colorBy,
                    this.baseModificationThreshold
                )
            }
            ...
            function drawBlock(block, b) {
                ...
                if (lastBlock) {
                    ...
                    if (lastBlockPositiveStrand) {
                        ...
                        yListPixel = [y, y, y + alignmentHeight / 2.0, y + alignmentHeight, y + alignmentHeight, y]
                    }
                ...
                // Internal block
                    ...
                    IGVGraphics.fillRect(ctx, blockStartPixel, y, blockWidthPixel, alignmentHeight, {
            ...
            function renderBlockOrReadChar(context, bpp, bbox, color, char) {
                ...
                if (bpp <= threshold && bbox.height >= 8) {
                    ...
                    IGVGraphics.strokeText(
                        context,
                        char,
                        center - context.measureText(char).width / 2,
                        fontHeight - 1 + bbox.y,
                        { strokeStyle: color }
                    )
                } else {
    ...
    colorByCB(menuItem, showCheck) {
        ...
        if (menuItem.key !== "tag") {
            ...
            function dialogPresentationHandler(ev) {
                const tag = this.alignmentTrack.colorBy.startsWith("tag:")
                    ? this.alignmentTrack.colorBy.substring(4)
                    : ""
    ...
    contextMenuItemList(clickState) {
        ...
        const position = `${viewport.referenceFrame.chr}:${StringUtils.numberFormatter(
            Math.floor(clickState.genomicLocation) + 1
        )}`
        list.push({
        ...
        if (clickedObject) {
            const showSoftClips = this.showSoftClips
            const clickedAlignment =
                typeof clickedObject.alignmentContaining === "function"
            ...
            if (clickedAlignment) {
                        ...
                        click: () => {
                            if (clickedAlignment.mate) {
                                ...
                                if (chromosomeObject) {
                                    ...
                                    this.browser.addMultiLocusPanel(
                                    ...
                                    this.browser.alert.present(
                                        `Reference does not contain chromosome: ${clickedAlignment.mate.chr}`
                                    )
                                }
                            }
                        },
                ...
                if (softClips.left && softClips.left.len > 0) {
                    list.push({
                        ...
                        click: () => {
                            const clippedSequence = clickedAlignment.seq.substring(
                                softClips.left.seqOffset,
                                softClips.left.seqOffset + softClips.left.len
                            )
                ...
                if (softClips.right && softClips.right.len > 0) {
                    list.push({
                        ...
                        click: () => {
                            const clippedSequence = clickedAlignment.seq.substring(
                                softClips.right.seqOffset,
                                softClips.right.seqOffset + softClips.right.len
                            )
                            this.browser.alert.present(clippedSequence)
                        },
                    })
                ...
                if (seqstring && "*" !== seqstring) {
                    ...
                    if (seqstring.length < maxSequenceSize) {
                        list.push({
                            ...
                            click: () => {
                                const sequence = clickedAlignment.isNegativeStrand()
                                    ? reverseComplementSequence(seqstring)
                                    : seqstring
                    ...
                    if (
                        softClips.left &&
                        softClips.left.len > MINIMUM_BLAT_LENGTH &&
                        softClips.left.len < maxSequenceSize
                    ) {
                        list.push({
                            label: "BLAT left soft-clipped sequence",
                            click: () => {
                                const clippedSequence = seqstring.substring(
                                    softClips.left.seqOffset,
                                    softClips.left.seqOffset + softClips.left.len
                                )
                                const sequence = clickedAlignment.isNegativeStrand()
                                    ? reverseComplementSequence(clippedSequence)
                                    : clippedSequence
                    ...
                    if (
                        softClips.right &&
                        softClips.right.len > MINIMUM_BLAT_LENGTH &&
                        softClips.right.len < maxSequenceSize
                    ) {
                        list.push({
            ...
            for (let group of features.packedGroups.values()) {
                ...
                if (offsetY > group.pixelTop && offsetY <= group.pixelBottom) {
                    ...
                    if (packedAlignmentsIndex >= 0 && packedAlignmentsIndex < group.length) {
                        ...
                        const clicked = alignmentRow.alignments.filter((alignment) =>
                            alignment.containsLocation(genomicLocation, this.showSoftClips)
                        )
    ...
    get minTemplateLength() {
        return this.minTLEN !== undefined
            ? this.minTLEN
            : this.parent._pairedEndStats
            ? this.parent._pairedEndStats.minTLEN
            : 0
    }

    get maxTemplateLength() {
        return this.maxTLEN !== undefined
            ? this.maxTLEN
            : this.parent._pairedEndStats
            ? this.parent._pairedEndStats.maxTLEN
            : 1000
    }
